...
# ==============================================================================
# COMMENTED OUT - These tasks are not currently used and have broken imports
...
# ==============================================================================
# ACTIVE TASK - Currently used for workspace ZIP uploads
# ==============================================================================



@celery.task(bind=True, max_retries=3)
def sync_workspace_session(self, project_id: int, session_id: str, initiated_by: int = None):
    """
    Task asincrono per sincronizzare una sessione workspace con GitHub.
    Gestisce il caricamento massivo di file in background.
    """
    ...
    with app.app_context():
        try:
            logger.info(f"Starting async sync for project {project_id}, session {session_id}")
            
            project = Project.query.get(project_id)
            if not project:
                logger.error(f"Project {project_id} not found during async sync")
                return
                
            service = WorkspaceSyncService()
            result = service.sync_session(project, session_id, initiated_by=initiated_by)
            
            logger.info(f"Async sync completed for session {session_id}: {result.get('status')}")
            return result
            
        except Exception as exc:
            logger.error(f"Error in async sync for session {session_id}: {exc}")
            # Retry con backoff esponenziale (delay 10s, 20s, 40s...)
