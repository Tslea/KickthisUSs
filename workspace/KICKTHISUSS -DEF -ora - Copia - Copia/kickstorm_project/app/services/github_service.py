...
class GitHubService:
    ...
    def upload_file(
        ...
        try:
            ...
            # Ensure content is bytes (PyGithub handles Base64 encoding internally)
            if isinstance(content, str):
                content = content.encode('utf-8')
            ...
            try:
                ...
                # Note: PyGithub's update_file expects raw content (bytes/str), not Base64
                result = repo.update_file(
                    path=file_path,
                    message=commit_message,
                    content=content,  # Pass raw bytes, PyGithub encodes internally
                ...
                # Note: PyGithub's create_file expects raw content (bytes/str), not Base64
    ...
    def create_pr_from_zip(
        ...
        try:
            ...
            pr_body = self._generate_pr_body(solution, user_info, files_committed, commit_messages)
    ...
    def _generate_pr_body(
        ...
        """Genera il body della Pull Request con template professionale"""
        task = solution.task
        
        body = f"""## 🚀 Solution Submission
...
"""
        ...
        # Aggiungi lista commit (primi 10)
        for i, msg in enumerate(commit_messages[:10], 1):
            body += f"{i}. {msg}\n"
        
        if len(commit_messages) > 10:
            body += f"\n...and {len(commit_messages) - 10} more files\n"
        
        body += f"""
---

**🤖 Auto-generated by KICKTHISUSS Platform**  
