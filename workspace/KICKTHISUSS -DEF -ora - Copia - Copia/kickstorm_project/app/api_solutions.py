...
def approve_solution_api(project_id, solution_id):
    ...
    try:
        with db_transaction():
            ...
            try:
                # Check if project uses new shares system
                if project.uses_shares_system():
                    from app.services.share_service import ShareService
                    share_service = ShareService()
                    granted_shares = share_service.distribute_task_completion_shares(solution)
                    
                    if granted_shares:
                        shares_count = float(granted_shares.shares_count)
                        percentage = granted_shares.get_percentage()
                        current_app.logger.info(
                            f'✅ Granted {shares_count} shares ({percentage:.2f}%) to user {solution.submitted_by_user_id} for task {task.id}'
                        )
                        
                        shares_notification = Notification(
                            user_id=solution.submitted_by_user_id,
                            project_id=project_id,
                            type='shares_granted',
                            message=f'🎉 Hai guadagnato {shares_count:.0f} shares ({percentage:.2f}% partecipazione) sul progetto "{project.name}" completando il task "{task.title}"!',
                            link=url_for('projects.project_detail', project_id=project_id)
                        )
                        db.session.add(shares_notification)
                        current_app.logger.info(f'📬 Notification created for shares grant to user {solution.submitted_by_user_id}')
                else:
                    # Use old equity system (backward compatibility)
                    ...
                    if granted_equity:
                        ...
                        equity_notification = Notification(
                            ...
                            project_id=project_id,
                            type='equity_granted',
                            message=f'🎉 Hai guadagnato {task.equity_reward}% shares sul progetto "{project.name}" completando il task "{task.title}"!',
                            link=url_for('projects.project_detail', project_id=project_id)
                        )
                ...
                current_app.logger.warning(
                    f'⚠️ Could not distribute shares/equity: {str(distribution_error)}'
                )
            ...
            NotificationService.notify_solution_approved(solution, task, project)
        return jsonify({
                        ...
                        try:
                            ...
                            if granted_equity:
                                ...
                                equity_notification = Notification(
                                    ...
                                    project_id=project.id,
                                    type='equity_granted',
                                    message=f'🎉 Hai vinto la votazione e guadagnato {task.equity_reward}% shares sul progetto "{project.name}" per il task "{task.title}"!',
