...
def create_project_api():
    ...
    try:
        ...
        # Creates initial shares for:
        # - Creator (10% default from config CREATOR_SHARES_PERCENTAGE)
        # - Platform/KickthisUSs (5% default from config PLATFORM_SHARES_PERCENTAGE)
        try:
            from app.services.share_service import ShareService
            share_service = ShareService()
            result = share_service.initialize_project_shares(new_project)
            db.session.commit()  # Commit the shares
            current_app.logger.info(
                f'Initialized phantom shares system for project {new_project.id} - '
                f'Creator: {result.get("creator_share")}, Platform: {result.get("platform_share")}'
            )
        except Exception as e:
            db.session.rollback()
    ...
    try:
        ...
        for task_data in suggested_tasks:
            ...
            if task_type not in ALLOWED_TASK_TYPES:
                task_type = 'code' # Default sicuro se l'IA fornisce un valore non valido
