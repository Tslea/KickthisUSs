...
def suggest_ai_task_api(project_id: int):
    ...
    try:
        # ⭐ NEW: Get progressive context from Hub Agents if available
        hub_context = ""
        try:
            from app.hub_agents.context_service import get_context_for_task_generation
            hub_context = get_context_for_task_generation(project_id)
            if hub_context:
                current_app.logger.info(f"Added Hub Agent context for task generation ({len(hub_context)} chars)")
        except ImportError:
            pass
        except Exception as ctx_error:
            current_app.logger.warning(f"Could not get Hub context for tasks: {ctx_error}")
        ...
        try:
            ...
            for t in existing_tasks:
                existing_tasks_list.append({
                    ...
                    'equity_reward': t.equity_reward,
                    'status': t.status
                })
            current_app.logger.info(f"Found {len(existing_tasks_list)} existing tasks for deduplication")
        except Exception as e:
            current_app.logger.warning(f"Could not get existing tasks: {e}")
...
"""
        ...
        suggested_tasks = generate_suggested_tasks(
            ...
            existing_tasks=existing_tasks_list,  # ⭐ Pass existing tasks to avoid duplicates
